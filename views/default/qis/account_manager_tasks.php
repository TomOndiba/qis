...
if (elgg_is_logged_in()) {
	...
	foreach ($quota_requests as $quota_request) {
		...
		if ($result) {
			...
			$view_form = elgg_view('input/form', array('body' => $mod_but, 'action' => "{$CONFIG->url}qis/manage_quota_request/$quota_request->container_guid/$quota_request->guid"));
	...
	foreach ($resident_permit_requests as $resident_permit_request) {
		...
		if ($result) {
			...
			$mod_but = elgg_view('input/submit', array('value' => elgg_echo('View')));
			$view_form = elgg_view('input/form', array('body' => $mod_but, 'action' => "{$CONFIG->url}qis/$form/$resident_permit_request->container_guid/$resident_permit_request->guid"));
