		...
		if ($object->qistype == 'resident_permit_request') {
			...
			if ($submitter->qisusertype == 'Client Portal Administrator') {
				if ($object->in_quatar == 'yes') {
						...
						$content .= '<h3>'.elgg_echo('need_visa_info_for ')."$user->name</h3>";
					}
			...
			if ($submitter->qisusertype == 'Immigration Agency Portal Coordinator') {
				...
				if (is_array($quantity)){
					...
					$num_lines = count($quantity);
        				for ($i=0;$i<$num_lines;$i++) {
						if ($object->status[$i] == 'Rejected') {
                					$content .= '<h3>'.elgg_echo('Quota Rejected for').$object->guid.'</h3>';
						}
					}
				} else {
				...
				if (is_array($quantity)){
					...
					$num_lines = count($quantity);
        				for ($i=0;$i<$num_lines;$i++) {
						if ($object->status[$i] == 'Approved') {
                					$content .= '<h3><font color="green">'.elgg_echo('Quota approved for').$object->guid.'</font></h3>';
						}
					}
				} else {
					if ($object->status == 'Approved') {
                				$content .= '<h3><font color="gree">'.elgg_echo('Quota approved for ').$object->guid.'</font></h3>';
					}
				}
			}
		}
