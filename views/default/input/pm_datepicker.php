...
END;
    ...
    if($newval){
	    if((date($dateformat, $newval) !== false) && (date($dateformat, $newval) != date($dateformat, 0))){
			// probably a timestamp, we can format it
			$dateval =  strftime($dateformat, $newval);
		} elseif(($new = strtotime($newval)) !== false){
			// time in date format
...
<div>
	...
	<input type="text" READONLY id="<?php echo $internal_id; ?>" value="<?php echo $dateval; ?>" style="width:70px"/>
</div>
